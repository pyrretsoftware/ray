...
//go:embed options/dials_config_base.txt
var Base string

func main() {
	fmt.Println("raydoc generates documentation from a file at ./workspace/types.go or ./workspace/types.go_")

	fset := token.NewFileSet()
	file, err := parser.ParseFile(fset, "./workspace/types.go", nil, parser.ParseComments)
	if err != nil {
		file, err = parser.ParseFile(fset, "./workspace/types.go_", nil, parser.ParseComments)
		if err != nil {
			log.Fatal(err)
		}
	}
	...
	err = os.WriteFile("./workspace/output.md", []byte(output), 0600)
	if err != nil {
		log.Fatal(err)
	}
